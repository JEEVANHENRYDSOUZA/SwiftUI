...
struct ContentView: View {
    ...
    var body: some View {
        ...
        VStack(spacing:40){
            Button(action:{}){
                Text("Solid Button").padding().foregroundStyle(.white)
                    .background(Color.purple).clipShape(RoundedRectangle(cornerRadius: 16))
            }
            
            Button(action:{}){
                
                
                Text("Button With Shadow").padding(12).foregroundStyle(.white).background(.purple).shadow(color:Color.purple,radius:20, y:5)
            }
            
            
            Button(action:{}){
                Text("Button With rounded   Ends").padding(12).foregroundStyle(.white).background(.purple,in:Capsule())
            }
